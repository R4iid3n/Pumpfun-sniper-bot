...
=============================================================
Detects new pump.fun tokens via Helius Enhanced WebSocket
(transactionSubscribe at 'processed' commitment) — fires ~1 slot (~400ms)
before pumpportal.fun emits its event.  Falls back to pumpportal if the
Helius atlas endpoint is unavailable or the plan doesn't support it.
...
async def find_token_pumpportal() -> dict | None:
    ...
    """
    print(f"\n  Falling back to pumpportal.fun WebSocket...")
